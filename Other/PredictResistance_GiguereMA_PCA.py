...
for i, varnames in enumerate(feature_names):
    ...
    if xs[i] >= 0.13 and 0.03 >= ys[i] >= -0.03:
        plt.text(xs[i], ys[i], varnames)
        listFeaturesPC.append(varnames)
...
plt.title('2D Biplot')
plt.tight_layout()
plt.savefig('PCA_2Dbiplot.svg')
plt.show()

listFeaturesPC.append('aa1')
AAproperties = AAproperties[listFeaturesPC]
