...
def GenerateFeatureGraphML(strain, locus, drug):
    ...
    TrainData = ConditionMaster.groupby(['strain', 'locus', 'compound', 'aa_pos', 'alt_aa'])[['median_s']].median().reset_index()
    TestData = TestMaster.groupby(['strain', 'locus', 'compound', 'aa_pos', 'alt_aa'])[['median_s']].median().reset_index()
    ...
    # Merge data frames.
    TrainMerged = pd.merge(left=TrainData, right=AAproperties, how='inner', indicator='location', suffixes=(None, '_singles'), on='alt_aa')
    TestMerged = pd.merge(left=TestData, right=AAproperties, how='inner', indicator='location', suffixes=(None, '_singles'), on='alt_aa')

    # creating train and test sets
    X_train = TrainMerged.drop(columns=['strain', 'locus', 'compound', 'median_s', 'location', 'alt_aa'])
    y_train = TrainMerged['median_s']

    X_test = TestMerged.drop(columns=['strain', 'locus', 'compound', 'median_s', 'location', 'alt_aa'])
    ...
    plt.ylabel('feature')
    plt.savefig(f'ML_NEW_{strain}_{locus}_{drug}.png')
