...
ConditionMaster = ConditionMaster.loc[ConditionMaster['compound'] == 'anidulafungin']
...
shap_df = pd.DataFrame(shap_test.values, columns=shap_test.feature_names, index=X_test_s_df.index)

columns = shap_df.apply(np.abs).mean().sort_values(ascending=False).indexfig, ax = plt.subplots(1, 2, figsize=(11,4))
sns.barplot(data=shap_df.apply(np.abs), orient='h', ax=ax[0])
ax[0].set_title("Mean absolute shap value")
sns.boxplot(data=shap_df, orient='h', ax=ax[1])
