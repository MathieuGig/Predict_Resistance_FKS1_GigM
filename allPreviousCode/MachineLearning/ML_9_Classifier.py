...
def parse_selection_coefficient(s):
    if s > 0.5:
        return 'resistant'
    elif s < -0.5:
        return 'deleterious'
...
ModelData['resistance'] = ModelData.median_s.apply(lambda x: parse_selection_coefficient(x))
...
X = merged.drop(columns=['strain', 'locus', 'compound', 'median_s', 'aa_seq', 'location', 'resistance'])
