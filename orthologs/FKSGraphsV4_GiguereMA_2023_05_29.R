...
{
    Input <- append(Input, list(TableauFKS1$Hotspot1[TableauFKS1$Species == as.character(i) & TableauFKS1$HS1_in_oPools == TRUE]))   
}
...
upset(fromList(Input), nsets = 10 , order.by = "freq",
      ...
      matrix.color = "orange3", main.bar.color = "orange3", sets.bar.color = "orange3")
...
for (i in unique(TableauFKS1$Species))
{
    Input <- append(Input, list(TableauFKS1$Hotspot2[TableauFKS1$Species == as.character(i) & TableauFKS1$HS2_in_oPools == TRUE]))   
}
names(Input) <- unique(TableauFKS1$Species)
...
{
    Input <- append(Input, list(TableauFKS2$Hotspot1[TableauFKS2$Species == as.character(i) & TableauFKS2$HS1_in_oPools == TRUE]))   
}
...
upset(fromList(Input), nsets = 10 , order.by = "freq",
      ...
      sets.x.label = "hotspot1 sequence per specie",
      matrix.color = "green3", main.bar.color = "green3", sets.bar.color = "green3")
...
for (i in unique(TableauFKS2$Species))
{
    Input <- append(Input, list(TableauFKS2$Hotspot2[TableauFKS2$Species == as.character(i) & TableauFKS2$HS2_in_oPools == TRUE]))   
}
names(Input) <- unique(TableauFKS2$Species)
