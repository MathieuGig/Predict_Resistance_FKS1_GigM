...
shortMaster = master[['strain', 'locus', 'pool_type', 'seq_type', 'compound', 'median_s', 'alt_aa', 'aa_pos', 'Nham_aa', 'WT']]

########################################################################################################################

def GenerateFeatureGraphML(strain, locus, drug, seq_type='ortho'):

    orthologMaster = shortMaster.loc[(shortMaster['strain'] == strain) & (shortMaster['locus'] == locus) & (shortMaster['compound'] == drug) & (shortMaster['seq_type'] == seq_type)]

    ModelData = orthologMaster.groupby(['strain', 'locus', 'compound', 'alt_aa', 'aa_pos', 'Nham_aa'])[['median_s']].median().reset_index()
    ...
    # Merge data frames.
    merged = pd.merge(left=ModelData, right=AAproperties, how='inner', indicator='location', suffixes=(None, '_singles'),
                      on='alt_aa')

    X = merged.drop(columns=['strain', 'locus', 'compound', 'median_s', 'location', 'alt_aa'])
    y = merged['median_s']
    ...
    grid = {'n_estimators': [300, 400, 500, 600, 700, 800, 900],
    ...
    plt.ylabel('feature')
    plt.savefig(f'ML_{seq_type}_{strain}_{locus}_{drug}.png')
